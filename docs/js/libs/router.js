...
(function () {
  ...
  var makeRoutes = function makeRoutes(content) {
    var recurse = function recurse(content, routes) {
      if (content) {
  ...
  try {
    ...
    for (var i = 0; i < route.length - 1; ++i) {
      ...
      scripts.push("js/content/" + trace.join("/") + "/" + route[i] + "-inherit.js");
      // Add a breadcrumb to the navbar for this level.
      nav.push("<li><a href=\"index.html?page=" + trace.join(",") + "\">" + level["children"][route[i]]["displayName"] + "</a></li>");
    ...
    scripts.push("js/content/" + trace.slice(0, trace.length).join("/") + "/" + route[route.length - 1] + ".js");
    // Add a breadcrumb to the navbar for this level.
    var title = level["children"][route[i]]["displayName"];
    nav.push("<li class=\"active\"><a href=\"index.html?page=" + routeJoined + "\">" + title + "</a></li>");
  } catch (e) {
